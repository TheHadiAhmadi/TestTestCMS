...
public class LayoutProcessor(ITemplateRenderingProvider renderingProvider) : ILayoutProcessor
{
    ...
    private static List<Dictionary<string, string>> ExtractComponents(string htmlContent)
    {
        ...
        // Regular expression to find tags with TestTestCMS attribute
        var tagPattern = new Regex(@"<(?<tag>\w+)(?<attributes>[^>]*?\sTestTestCMS\s[^>]*?)\/?>", RegexOptions.IgnoreCase);
        ...
        foreach (Match match in matches)
        {
            ...
            //// Adding the "TestTestCMS" attribute as its presence is what we are checking
            //if (!attributes.ContainsKey("TestTestCMS"))
            //{
            //    attributes["TestTestCMS"] = string.Empty;
            //}
