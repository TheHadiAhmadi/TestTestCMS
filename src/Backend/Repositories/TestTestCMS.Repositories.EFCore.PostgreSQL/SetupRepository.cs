...
namespace TestTestCMS.Repositories.EFCore.PostgreSQL;

public class SetupRepository(TestTestCMSDbContext dbContext) : ISetupRepository
{
    public async Task<bool> Initialized(CancellationToken cancellationToken = default)
    {
        ...
        var commandText = @"
            SELECT COUNT(*) 
            FROM pg_tables 
            WHERE schemaname = 'public' AND tablename = 'GlobalSettings';";
    ...
    public async Task<bool> InitializeDb(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var resourceName = "TestTestCMS.Repositories.EFCore.PostgreSql.Setup.sql";
